...
import { getSwapQuote, buildSwapTransaction, estimateUsdcOutput, type JupiterQuote } from '../services/jupiter';
import {
  MINT_SOURCE_TOKENS,
  CR_STABLE,
  CR_MINT_FEE_RATE,
...
export function MintCRStable() {
  const { publicKey: adapterPubkey, signTransaction: adapterSign, signAllTransactions } = useWallet();
  ...
  const [selectedSource, setSelectedSource] = useState<MintableToken>(MINT_SOURCE_TOKENS[0]);
  ...
  // For stablecoins: 1:1 minus fee. For SOL: use Jupiter quote.
  const mintAmount = useMemo(() => {
    if (!validAmount) return 0;
    if (isSOL) {
      return solQuote ? solQuote.usdcAmount * (1 - CR_MINT_FEE_RATE) : 0;
    }
    return amountNum * (1 - CR_MINT_FEE_RATE);
  }, [amountNum, validAmount, isSOL, solQuote]);

  const feeAmount = useMemo(() => {
    if (!validAmount) return 0;
    if (isSOL) {
      return solQuote ? solQuote.usdcAmount * CR_MINT_FEE_RATE : 0;
    }
    return amountNum * CR_MINT_FEE_RATE;
  }, [amountNum, validAmount, isSOL, solQuote]);

  // Fetch Jupiter quote when SOL is selected and amount changes
  useEffect(() => {
  ...
  const handleMint = async () => {
    ...
    if (!validAmount || amountNum < CR_MINT_MINIMUM) {
      toast.error(`Minimum mint amount is $${CR_MINT_MINIMUM}`);
      return;
    }
  ...
  /**
   * USDT/USDC direct path: send stablecoin through the gateway for 1:1 CR Stable mint.
   */
  const buildSplDepositTx = async (splMintAddress: string, amountDecimal: number) => {
    ...
    if (!ataInfo) {
      throw new Error(`No ${sym} token account found. Please acquire ${sym} first.`);
    }
  ...
  const handleStableMint = async () => {
    ...
    setActiveTransfer({
      ...
      tokenSymbol: `${selectedSource.symbol} → ${CR_STABLE.symbol}`,
      useZk: false,
    });
    ...
    bridgeApi.registerTransfer({
      ...
      recipient: recipientDcc,
      amount: amountNum.toString(),
      amountFormatted: `${amountNum} ${selectedSource.symbol} → ${CR_STABLE.symbol}`,
      splMint: selectedSource.splMint,
    ...
    connection.confirmTransaction(
      ...
      if (conf.value.err) {
        ...
        toast.success('Deposit confirmed! CR Stable will be minted shortly.');
      }
    });
  };

  /**
   * SOL path: swap SOL → USDC via Jupiter, then send USDC through the gateway.
   */
  const handleSOLMint = async () => {
    ...
    const { blockhash: swapBh, lastValidBlockHeight: swapLvbh } =
    ...
    // 4. Now deposit the USDC through the gateway as a stablecoin mint
    const usdcAmountRaw = Number(quote.outAmount); // already in USDC smallest units (6 dec)
    const usdcAmountDecimal = usdcAmountRaw / 1e6;

    const usdcSource = MINT_SOURCE_TOKENS.find((t) => t.symbol === 'USDC')!;
    const { tx, transferId, blockhash, lastValidBlockHeight } = await buildSplDepositTx(
    ...
    setActiveTransfer({
      ...
      tokenSymbol: `SOL → ${CR_STABLE.symbol}`,
      useZk: false,
    });
    ...
    bridgeApi.registerTransfer({
      ...
      recipient: recipientDcc,
      amount: usdcAmountDecimal.toString(),
      amountFormatted: `${amountNum} SOL → ${mintAmount.toFixed(2)} ${CR_STABLE.symbol}`,
      splMint: usdcSource.splMint,
      sourceTxHash: signature,
      direction: 'sol_to_dcc',
    }).catch(() => {});
    ...
    <div className="space-y-6">
      ...
      <div className="card space-y-5">
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">Mint fee ({CR_MINT_FEE_DISPLAY})</span>
            {feeAmount > 0 ? (
          ...
          {isSOL && (
            <div className="flex justify-between text-sm">
              ...
              <span className="text-xs text-blue-300">SOL → USDC (Jupiter) → Gateway → {CR_STABLE.symbol}</span>
            </div>
          )}
          ...
          <div className="flex justify-between text-sm font-medium">
            ...
            <span className="text-emerald-400 text-lg">
              {mintAmount > 0 ? `${mintAmount.toFixed(2)} ${CR_STABLE.symbol}` : `0 ${CR_STABLE.symbol}`}
            </span>
          </div>
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2">
          <span className="w-5 h-5 rounded bg-emerald-600 flex items-center justify-center text-[10px] text-white font-bold">?</span>
          How {CR_STABLE.symbol} Works
        </h3>
        <div className="space-y-3">
          {[
            ...
            {
              icon: '💎',
              title: `Mint ${CR_STABLE.symbol}`,
              desc: `${CR_STABLE.symbol} is minted 1:1 on DecentralChain, fully backed by the locked reserves.`,
            },
            {
              icon: '🔄',
              title: 'Redeem Anytime',
              desc: `Burn ${CR_STABLE.symbol} on DCC to unlock your USDT/USDC on Solana.`,
            },
          ].map((step, i) => (
      ...
      {/* Reserve Info */}
      <div className="card bg-emerald-950/20 border-emerald-500/10">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-emerald-300">Reserve Backing</h3>
        </div>
        <p className="text-xs text-gray-400 leading-relaxed mb-3">
          Every {CR_STABLE.symbol} is backed 1:1 by USDT/USDC locked in the Solana bridge vault.
