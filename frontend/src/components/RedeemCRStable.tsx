...
export function RedeemCRStable() {
  ...
  const redeemTokens = MINT_SOURCE_TOKENS.filter((t) => !t.requiresSwap);
  ...
  const feeAmount = useMemo(() => {
    ...
    return amountNum * CR_REDEEM_FEE_RATE;
  ...
  const handleRedeem = async () => {
    ...
    if (!validAmount || amountNum < CR_MINT_MINIMUM) {
      toast.error(`Minimum redeem amount is $${CR_MINT_MINIMUM}`);
      return;
    }
    ...
    try {
      ...
      toast.success(`${CR_STABLE.symbol} burn instruction generated — sign in DCC wallet`);
      ...
      setActiveTransfer({
        ...
        splMint: selectedOutput.splMint,
        tokenSymbol: `${CR_STABLE.symbol} → ${selectedOutput.symbol}`,
      });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      {/* Hero */}
      <div className="card bg-gradient-to-br from-orange-950/40 to-gray-900/40 border-orange-500/20">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl bg-orange-600 flex items-center justify-center font-bold text-lg text-white shadow-lg shadow-orange-500/30">
            ↩
          </div>
          <div>
            <h1 className="text-xl font-bold">Redeem {CR_STABLE.symbol}</h1>
            <p className="text-xs text-gray-400">Burn {CR_STABLE.symbol} to unlock USDC/USDT on Solana</p>
          </div>
        </div>
        <p className="text-sm text-gray-400 leading-relaxed">
          Burn your {CR_STABLE.symbol} on DecentralChain and receive the equivalent USDC or USDT
      ...
      <div className="card space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            {CR_STABLE.symbol} Amount to Burn
          </label>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border transition-all ${
                  selectedOutput.splMint === token.splMint
                    ? 'border-orange-500 bg-orange-600/20 text-white shadow-md shadow-orange-500/10'
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>{amount || '0'} {CR_STABLE.symbol}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">Redeem fee ({CR_REDEEM_FEE_DISPLAY})</span>
            {feeAmount > 0 ? (
        ...
        {publicKey ? (
          ...
          >
            {isSubmitting ? (
              ...
              `Redeem ${CR_STABLE.symbol}`
            )}
          </button>
        ) : (
          <div className="text-center text-gray-500 text-sm py-3">
            Connect your Solana wallet above to redeem {CR_STABLE.symbol}
          </div>
        )}
      </div>
    </div>
